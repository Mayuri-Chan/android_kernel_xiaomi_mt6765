...
struct upgrade_fw fw_list[] = {
    {FTS_VENDOR_ID, FTS_MAKER_ID, fw_file, sizeof(fw_file)},
    {FTS_VENDOR_ID2, FTS_MAKER_ID2, fw_file2, sizeof(fw_file2)},
    {FTS_VENDOR_ID3, FTS_MAKER_ID3, fw_file3, sizeof(fw_file3)},
    {FTS_VENDOR_ID4, FTS_MAKER_ID4, fw_file4, sizeof(fw_file4)},
};
...
static int fts_fwupg_get_fw_file(struct fts_ts_data *ts_data)
{
    ...
    ret = fts_flash_read(ts_data->client, 0xD7A1, &maker_id, 1);
    ...
    for (i = 0; i < FTS_GET_VENDOR_ID_NUM; i++) {
        fw = &fw_list[i];
        if (vendor_id == fw->vendor_id && maker_id == fw->maker_id) {
            FTS_INFO("get fw file successful(vendor_id: 0x%x, maker_id: 0x%x)", vendor_id, maker_id);
            break;
        }
    }
