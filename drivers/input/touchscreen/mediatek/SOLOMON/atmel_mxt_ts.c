...
struct mxt_data {
	...
	u8 *msg_buf;
	u8 t6_status;
	/*array used for storing selftest result*/
	u8 test_result[6];
	bool update_input;
...
/* update config during the boot up */
static int mxt_download_config(struct mxt_data *data, const char *cfg_name)
{
...
static ssize_t doubletap2wake_dump(struct device *dev, struct device_attribute *attr, const char *buf, size_t count) {
	...
	if (dt2w_switch == 0 || dt2w_switch == 1) {
		mxt_i2c_data->mxt_wakeup_gesture_enable = dt2w_switch;
	}
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* update the pdata config file name to data->cfg_name */
	...
	if (IS_ERR(thread)) {
		...
		printk(" mxt_probe, main thread, step 1, %s: failed to create kernel thread: %ld\n", __func__, retval);
	}
	...
	if (IS_ERR(fw_upgrade_thread)) {
		...
		printk(" mxt: mxt_probe, step 4, %s: failed to create kernel thread: %ld\n", __func__, retval);
	}
	...
	error = sysfs_create_file(android_touch_kobj, &dev_attr_doubletap2wake.attr);
	if (error) {
