...
#include "cactus_hi556_sunny_Sensor.h"
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[] = {
	...
	{2592, 1944, 0, 0, 2592, 1944, 2592, 1944, 0, 0, 2592, 1944, 0, 0, 2592, 1944},
	// high speed video
	{2592, 1944, 16, 12, 2560, 1920, 640, 480, 0, 0, 640, 480, 0, 0, 640, 480},
	// slim video
...
static struct stCAM_CAL_DATAINFO_STRUCT sensor_eeprom_data = {
	...
	.deviceID = 2,
	.dataLength = 0x768,
	.sensorVendorId = 0x12AF556,
	.vendorByte = {1, 8, 9, 10},
	.dataBuffer = NULL
};

static struct stCAM_CAL_CHECKSUM_STRUCT cactus_hi556_sunny_checksum[] = {
...
static void cactus_hi556_sunny_table_write_cmos_sensor(const kal_uint16 *para, kal_uint32 len) {
...
static kal_uint16 cactus_hi556_sunny_otp_read(kal_uint16 otp_addr) {
	kal_uint16 i, data;
	i = otp_addr;
...
static void cactus_hi556_sunny_disable_otp(void) {
...
static void read_efuse_id(struct stCAM_CAL_DATAINFO_STRUCT *pData) {
	...
	for (i = 0; i < 9; i++) {
		buf[i] = cactus_hi556_sunny_otp_read(0x0001 + i);
...
static int cactus_hi556_sunny_otp_info(struct stCAM_CAL_DATAINFO_STRUCT *pData) {
	u32 flag_address;
	u8 vendor_id;

	int minfo_awb_width = 26, lsc_width = 1870;
	int index, i, buf_id = 0;
	...
	for (index = 2; index >= 0; index--) {
		flag_address = index * minfo_awb_width + 0x401;

		if (cactus_hi556_sunny_otp_read(flag_address) == 1) {
			vendor_id = cactus_hi556_sunny_otp_read(flag_address + 2);
			if ((pData->sensorVendorId >> 24) != vendor_id) {
				LOG_ERR("group(%d) failed: 0x%x != 0x%x\n", index, vendor_id, pData->sensorVendorId >> 24);
				return -1;
			} else {
				LOG_INF("group(%d) succeed: 0x%x\n", index, vendor_id);
			}

			for (i = 0; i < minfo_awb_width && buf_id < pData->dataLength; i++, buf_id++) {
				pData->dataBuffer[buf_id] = (kal_uint8) cactus_hi556_sunny_otp_read(flag_address + i);
	...
	for (index = 2; index >= 0; index--) {
		flag_address = index * lsc_width + 0x44F;

		if (cactus_hi556_sunny_otp_read(flag_address) == 1) {
			for (i = 0; i < lsc_width && buf_id < pData->dataLength; i++, buf_id++) {
				pData->dataBuffer[buf_id] = (kal_uint8) cactus_hi556_sunny_otp_read(flag_address + i);
	...
	// check eeprom data
	if (imgSensorCheckEepromData(pData, cactus_hi556_sunny_checksum) != 0) {
		LOG_ERR("OTP checksum failed\n");

		cactus_hi556_sunny_disable_otp();
...
kal_uint16 addr_data_pair_init_cactus_hi556_sunny[] = {
	0x0a00, 0x0000,
	0x0e00, 0x0102,
	0x0e02, 0x0102,
	0x0e0c, 0x0100,
...
static void sensor_init(void) {
	cactus_hi556_sunny_table_write_cmos_sensor(addr_data_pair_init_cactus_hi556_sunny, sizeof(addr_data_pair_init_cactus_hi556_sunny) / sizeof(kal_uint16));
}

kal_uint16 addr_data_pair_preview_cactus_hi556_sunny[] = {
	0x0b0a, 0x8259,
	0x0f30, 0x6e25,
	0x0f32, 0x7167,
	0x004a, 0x0100,
...
static void preview_setting(void) {
	cactus_hi556_sunny_table_write_cmos_sensor(addr_data_pair_preview_cactus_hi556_sunny, sizeof(addr_data_pair_preview_cactus_hi556_sunny) / sizeof(kal_uint16));
}

kal_uint16 addr_data_pair_capture_fps_cactus_hi556_sunny[] = {
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7067,
	0x004a, 0x0100,
...
kal_uint16 addr_data_pair_capture_30fps_cactus_hi556_sunny[] = {
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7067,
	0x004a, 0x0100,
...
static void capture_setting(kal_uint16 current_fps) {
	if (current_fps == 300) {
		cactus_hi556_sunny_table_write_cmos_sensor(addr_data_pair_capture_30fps_cactus_hi556_sunny, sizeof(addr_data_pair_capture_30fps_cactus_hi556_sunny) / sizeof(kal_uint16));
	} else {
		cactus_hi556_sunny_table_write_cmos_sensor(addr_data_pair_capture_fps_cactus_hi556_sunny, sizeof(addr_data_pair_capture_fps_cactus_hi556_sunny) / sizeof(kal_uint16));
	}
}

kal_uint16 addr_data_pair_hs_video_cactus_hi556_sunny[] = {
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7267,
	0x004a, 0x0100,
...
static void hs_video_setting(void) {
	cactus_hi556_sunny_table_write_cmos_sensor(addr_data_pair_hs_video_cactus_hi556_sunny, sizeof(addr_data_pair_hs_video_cactus_hi556_sunny) / sizeof(kal_uint16));
}

kal_uint16 addr_data_pair_slim_video_cactus_hi556_sunny[] = {
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7167,
	0x004a, 0x0100,
...
static void slim_video_setting(void) {
	cactus_hi556_sunny_table_write_cmos_sensor(addr_data_pair_slim_video_cactus_hi556_sunny, sizeof(addr_data_pair_slim_video_cactus_hi556_sunny) / sizeof(kal_uint16));
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) {
	...
	for (i = 0; imgsensor_info.i2c_addr_table[i] != 0xFF; i++) {
		...
		size = cactus_hi556_sunny_otp_info(&sensor_eeprom_data);
...
static kal_uint32 set_test_pattern_mode(kal_bool enable) {
	if (enable) {
		write_cmos_sensor(0xA04, 0x143);
		write_cmos_sensor(0x200, 0x2);
	} else {
		write_cmos_sensor(0xA04, 0x142);
		write_cmos_sensor(0x200, 0x0);
	}
...
static kal_uint32 streaming_control(kal_bool enable) {
	...
	if (enable) {
		write_cmos_sensor(0xA00, 0x100);
	} else {
		write_cmos_sensor(0xA00, 0x0);
	}
	mdelay(10);
