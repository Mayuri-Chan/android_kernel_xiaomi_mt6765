...
static u8 imgSensorStaticEfuseID[IMGSENSOR_SENSOR_IDX_MAX_NUM][MAX_EFUSE_ID_LENGTH] ={{0},
                                   {0},
                                   {0},
                                   {0}};
	         ...
	         const char *buf, size_t count)
{
    ...
    if(totalBuf == NULL) {
        ...
        int retValue = 0;
		checkSum = 0;
        retValue = dual_cam_cal_read_total_data(sensorID,IMGSENSOR_SENSOR_IDX_MAIN,totalBuf);
