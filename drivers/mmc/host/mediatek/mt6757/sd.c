...
void msdc_dump_register_core(u32 id, void __iomem *base)
{
	...
	pr_debug("MSDC%d normal register\n", id);
...
void msdc_dump_dbg_register_core(u32 id, void __iomem *base)
{
	...
	memset(buffer, 0, PRINTF_REGISTER_BUFFER_SIZE);
	pr_debug("MSDC debug register [set:out]\n");
...
static void msdc_dump_mmc_info(struct msdc_host *host)
{
	...
	pr_debug("mmc%d: clock %dhz, timing %d\n", host->id, mmc->ios.clock,
		 mmc->ios.timing);
}
...
void msdc_dump_info(u32 id)
{
	...
	if (!sd_register_zone[id]) {
		pr_debug("msdc host<%d> is timeout when detect, so don't dump "
			 "register\n",
			 id);
		return;
	}
...
void msdc_raw_dump_info(u32 id)
{
	...
	pr_debug("====== msdc%d dump start ======\n", host->id);

	pr_debug("msdc%d need_tune = %d, %s\n", host->id, host->need_tune,
		 host->tuning_in_progress ? "tuning" : "not tuning");

	base = host->base;

	if (host->core_clkon == 0) {
		pr_debug("host%d enable clock\n", host->id);
...
void msdc_clr_fifo(unsigned int id)
{
	...
	if (MSDC_READ32(MSDC_DMA_CFG) & MSDC_DMA_CFG_STS) {
		pr_debug("<<<WARN>>>: msdc%d, clear FIFO when DMA active, "
			 "MSDC_DMA_CFG=0x%x\n",
		...
		msdc_retry((MSDC_READ32(MSDC_DMA_CFG) & MSDC_DMA_CFG_STS),
			   retry, cnt, id);
		if (retry == 0) {
			pr_debug("<<<WARN>>>: msdc%d, faield to stop DMA "
				 "before clear FIFO, MSDC_DMA_CFG=0x%x\n",
				 id, MSDC_READ32(MSDC_DMA_CFG));
			return;
		}
	}
		    ...
		    u32 hs400_div_dis)
{
	...
	do {
		...
		msdc_retry(!(MSDC_READ32(MSDC_CFG) & MSDC_CFG_CKSTB), retry,
			   cnt, host->id);
		if (retry == 0) {
			pr_debug("msdc%d host->onclock(%d)\n", host->id,
				 host->core_clkon);
...
#define msdc_irq_restore(val) MSDC_SET_BIT32(MSDC_INTEN, val)
		   ...
		   u8 *edge)
{
	...
	case TYPE_WRITE_CRC_EDGE:
		if (clock_mode == 3) {
			...
			MSDC_SET_FIELD(EMMC50_CFG0, MSDC_EMMC50_CFG_CRC_STS_SEL,
				       1);
		} else {
	...
	case TYPE_READ_DATA_EDGE:
		...
		if (mode == MSDC_SMPL_RISING || mode == MSDC_SMPL_FALLING) {
			...
			MSDC_SET_FIELD(MSDC_PATCH_BIT0, MSDC_PB0_RD_DAT_SEL,
				       mode);
		} else {
...
{
	msdc_set_smpl(host, clock_mode, host->hw->cmd_edge, TYPE_CMD_RESP_EDGE,
		      NULL);
...
static void msdc_check_dat_1to3_high(struct msdc_host *host)
{
	...
	while ((MSDC_READ32(MSDC_PS) & 0xE0000) != 0xE0000) {
		if (time_after(jiffies, polling_tmo)) {
			pr_info("msdc%d, some of device's pin, dat1~3 are "
				"stuck in low!\n",
				host->id);
			/*
			 * Don't remove 4 bit mode in FATCORY mode and META
			 * mode.
			 */
...
void msdc_gate_clock(struct msdc_host *host, int delay)
{
	...
	if (delay) {
		...
		N_MSG(CLK, "[%s]: msdc%d, clock gated done", __func__,
		      host->id);
	} else {
...
static int msdc_io_check(struct msdc_host *host)
{
	...
	while ((MSDC_READ32(MSDC_PS) & 0x70000) != 0) {
		if (time_after(jiffies, polling_tmo)) {
			...
			pr_info("msdc%d, one of device's dat(0~2) pin is stuck "
				"in high\n",
				host->id);
			pr_info("ps = 0x%x\n", MSDC_READ32(MSDC_PS));
...
static void msdc_set_timeout(struct msdc_host *host, u32 ns, u32 clks)
{
	...
	N_MSG(OPS, "msdc%d, Set read data timeout: %dns %dclks -> %d x 1048576 "
		   "cycles, mode:%d, clk_freq=%dKHz\n",
...
void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	pr_debug("msdc%d -> !!! Set<%dKHz> Source<%dKHz> -> sclk<%dKHz> "
		 "timing<%d> mode<%d> div<%d> hs400_div_dis<%d>",
		 host->id, hz / 1000, hclk / 1000, sclk / 1000, (int)timing,
		 mode, div, hs400_div_dis);
		...
		if (is_card_sdio(host) || (host->hw->flags & MSDC_SDIO_IRQ)) {
			...
			if (host->hw->host_function == MSDC_SD) {
				...
				if (host->mclk == HOST_MIN_MCLK) {
					...
					pr_notice(
					    "[%s]: msdc%d power off at clk "
					    "%dhz set block_bad_card = %d\n",
...
struct msdc_host *msdc_get_host(int host_function, bool boot, bool secondary)
{
	...
	if (host == NULL) {
		pr_debug("[MSDC] This host(<host_function:%d> "
			 "<boot:%d><secondary:%d>) isn't in MSDC host config "
			 "list",
...
int msdc_cache_ctrl(struct msdc_host *host, unsigned int enable, u32 *status)
{
	struct mmc_command cmd = {0};
	struct mmc_request mrq = {0};
	u32 err;
					   ...
					   u32 l_bypass_flush)
{
	...
	if (check_mmc_cmd47(cmd->opcode))
		sbc =
		    host->mmc->areq_que[(cmd->arg >> 16) & 0x1f]->mrq_que->sbc;
	else
#endif
	...
	if (sbc) {
		if ((host->error == 0) &&
			...
			if (g_cache_status == CACHE_UN_FLUSHED) {
				...
				N_MSG(CHE, "reliable/force prg write happened, "
					   "update g_cache_status = %d",
				      g_cache_status);
				N_MSG(CHE, "reliable/force prg write happened, "
					   "update g_flush_data_size = %lld",
				      g_flush_data_size);
				g_flush_data_size = 0;
			}
			...
			if (g_cache_status == CACHE_FLUSHED) {
				...
				N_MSG(CHE, "normal write happened, update "
					   "g_cache_status = %d",
				      g_cache_status);
			}
		...
		if (host->error == 0) {
			 ...
			 */
			g_cache_status = CACHE_FLUSHED;
			N_MSG(CHE, "flush happened, update g_cache_status = "
				   "%d, g_flush_data_size = %lld",
...
{
	if (g_flush_error_happened && check_mmc_cache_ctrl(host->mmc->card) &&
	    check_mmc_cache_flush_cmd(cmd)) {
		g_flush_error_count++;
		g_flush_error_happened = 0;
		ERR_MSG("the %d time flush error happened, g_flush_data_size = "
			"%lld",
		...
		if (g_flush_error_count >= MSDC_MAX_FLUSH_COUNT) {
			...
			pr_debug("msdc%d:flush cache error count = %d, Disable "
				 "cache\n",
		...
		if (host->autocmd & MSDC_AUTOCMD12) {
			...
			if (sbc) {
				...
				if (reg_blk_num != (sbc->arg & 0xFFFF))
					pr_debug("msdc%d: acmd23 arg(0x%x) "
						 "fail to match block "
						 "num(0x%x), "
						 "SDC_BLK_NUM(0x%x)\n",
						 host->id, sbc->arg,
	...
	if (host->chip_hw_ver == 0xcb00 && (rawcmd & (3 << 11))) {
		if (rawcmd & (1 << 13)) {
			...
			MSDC_SET_FIELD(SDC_FIFO_CFG,
				       (SDC_FIFO_CFG_RD_VALID_SEL |
					SDC_FIFO_CFG_WR_VALID_SEL),
				       0x3);
		} else {
...
/* clear all int flag */
		...
		if (time_after(jiffies, tmo)) {
			pr_debug("[%s]: msdc%d CMD<%d> polling_for_completion "
				 "timeout ARG<0x%.8x>",
...
#else
	if (intsts &
	    (MSDC_INT_CMDRDY | MSDC_INT_ACMD19_DONE | MSDC_INT_ACMDRDY)) {
#endif
		u32 *rsp = NULL;
		...
		if ((cmd->opcode != 19) && (cmd->opcode != 21))
			pr_debug("[%s]: msdc%d CMD<%d> MSDC_INT_RSPCRCERR "
				 "Arg<0x%.8x>",
				 __func__, host->id, cmd->opcode, cmd->arg);
		if (((mmc_resp_type(cmd) == MMC_RSP_R1B) ||
		     (cmd->opcode == 13)) &&
		    (host->hw->host_function != MSDC_SDIO)) {
			pr_debug("[%s]: msdc%d CMD<%d> ARG<0x%.8X> is R1B, CRC "
				 "not reset hw\n",
				 __func__, host->id, cmd->opcode, cmd->arg);
		} else {
			msdc_reset_hw(host->id);
		...
		if ((cmd->opcode != 19) && (cmd->opcode != 21))
			pr_debug(
			    "[%s]: msdc%d CMD<%d> MSDC_INT_CMDTMO Arg<0x%.8x>",
			    __func__, host->id, cmd->opcode, cmd->arg);
		...
		if ((mmc_resp_type(cmd) == MMC_RSP_R1B) ||
			...
			pr_debug("[%s]: msdc%d XXX CMD<%d> ARG<0x%.8X> is R1B, "
				 "TMO not reset hw\n",
				 __func__, host->id, cmd->opcode, cmd->arg);
		} else {
			msdc_reset_hw(host->id);
			     ...
			     unsigned long timeout)
{
	...
	MVG_EMMC_ERASE_MATCH(host, (u64)cmd->arg, delay_ms, delay_us, delay_ns,
			     cmd->opcode);
						   ...
						   unsigned long timeout)
{
	...
	if (intsts & cmdsts) {
		if (intsts & MSDC_INT_CMDRDY) {
			...
			cmd->error = (unsigned int)-EILSEQ;
			pr_debug("[%s]: msdc%d XXX CMD<%d> MSDC_INT_RSPCRCERR "
				 "Arg<0x%.8x>",
				 __func__, host->id, cmd->opcode, cmd->arg);
			/* msdc_dump_info(host->id); */
		} else if (intsts & MSDC_INT_CMDTMO) {
			cmd->error = (unsigned int)-ETIMEDOUT;
			pr_debug("[%s]: msdc%d XXX CMD<%d> MSDC_INT_CMDTMO "
				 "Arg<0x%.8x>",
				 __func__, host->id, cmd->opcode, cmd->arg);
			mmc_cmd_dump(host->mmc);
			msdc_dump_info(host->id);
				  ...
				  unsigned long timeout)
{
	...
	N_MSG(CMD, "		return<%d> resp<0x%.8x>", cmd->error,
	      cmd->resp[0]);
...
static void msdc_wr_dump_status(struct msdc_host *host)
{
	...
	for (i = 0; i <= retry; i++) {
		pr_debug("msdc%d checksum (%d) 0x%x\n", host->id, i,
			 host->dma_checksum[i]);
	}
}
...
static int msdc_wr_check_result(struct msdc_host *host)
{
	...
	for (i = 0; i < retry; i++) {
		for (j = i + 1; j <= retry; j++) {
			if (host->dma_checksum[i] == host->dma_checksum[j])
				return 0; /*PASS*/
		}
	}
...
static void msdc_wr_read_data(struct msdc_host *host, u32 *checksum)
{
	...
	while (sg) {
		...
		if ((subpage != 0) || (sg->offset != 0))
			N_MSG(OPS, "msdc%d: size or start not align %x, %x, "
				   "hmpage %lx,sg offset %x\n",
		...
		for (i = 0; i < totalpages; i++) {
			...
			if (!kaddr[i])
				pr_debug("msdc0:kmap failed %lx (%d)\n",
					 kaddr[i], i);
		}
...
void msdc_wr_check(struct msdc_host *host)
{
	...
	if (mmc->card) {
		main_md =
		    (struct _mmc_blk_data *)dev_get_drvdata(&mmc->card->dev);
	...
	case 2: /*FAIL*/
		...
		snprintf(str, sizeof(str), "msdc0 dma checksum fail!!! 0x%x\n",
			 host->dma_checksum[i]);
		aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT, str,
				       "msdc0 dma sram warning!");
	/*fall through*/
	case 0: /*PASS*/
	default:
...
int msdc_pio_read(struct msdc_host *host, struct mmc_data *data)
{
	...
	u32 count, size = 0;
	u32 wints =
	    MSDC_INTEN_DATTMO | MSDC_INTEN_DATCRCERR | MSDC_INTEN_XFER_COMPL;
	u32 ints = 0;
		...
		check_fifo1:
			...
			if ((msdc_rxfifocnt() >= MSDC_FIFO_THD) &&
				...
				if (ints & MSDC_INT_DATCRCERR) {
					ERR_MSG("[msdc%d] DAT CRC error "
						"(0x%x), Left DAT: %d bytes\n",
						host->id, ints, left);
					data->error = (unsigned int)-EILSEQ;
				} else if (ints & MSDC_INT_DATTMO) {
					ERR_MSG("[msdc%d] DAT TMO error "
						"(0x%x), Left DAT: %d bytes\n",
						host->id, ints, left);
					data->error = (unsigned int)-ETIMEDOUT;
				} else {
...
static void msdc_dma_start(struct msdc_host *host)
{
	...
	u32 wints = MSDC_INTEN_XFER_COMPL | MSDC_INTEN_DATTMO |
		    MSDC_INTEN_DATCRCERR | MSDC_INTEN_GPDCSERR |
		    MSDC_INTEN_BDCSERR;
...
static int msdc_dma_config(struct msdc_host *host, struct msdc_dma *dma)
{
	...
	case MSDC_MODE_DMA_BASIC:
		...
		N_MSG(DMA, "BASIC DMA len<%x> dma_address<%llx>", dma_len,
		      (u64)dma_address);
	...
	case MSDC_MODE_DMA_DESC:
		...
		for (j = 0; j < bdlen; j++) {
			...
			msdc_init_bd(&vbd, blkpad, dwpad, dma_address, dma_len);
			...
			vbd.chksum = 0;
			vbd.chksum =
			    (chksum ? msdc_dma_calcs((u8 *)(&vbd), 16) : 0);
			      ...
			      struct mmc_data *data)
{
	N_MSG(OPS, "CMD<%d> data<%s %s> blksz<%d> block<%d> error<%d>",
	      ...
	      ((data->flags & MMC_DATA_READ) ? "read " : "write"), data->blksz,
	...
	if (!(is_card_sdio(host) || (host->hw->flags & MSDC_SDIO_IRQ))) {
		if (!check_mmc_cmd2425(cmd->opcode) &&
			...
			N_MSG(
			    NRW,
			    "CMD<%3d> arg<0x%8x> Resp<0x%8x> data<%s> size<%d>",
...
int msdc_if_send_stop(struct msdc_host *host, struct mmc_request *mrq,
		      int req_type)
{
...
#endif
	{
		if ((mrq->cmd->error != 0) || (mrq->cmd->data->error != 0) ||
...
int msdc_do_request_prepare(struct msdc_host *host, struct mmc_request *mrq)
{
...
#if defined(CONFIG_MTK_EMMC_CQ_SUPPORT) || defined(MTK_MSDC_USE_CACHE) ||      \
	defined(CONFIG_MTK_EMMC_SUPPORT_OTP)
	struct mmc_command *cmd = mrq->cmd;
#endif
	...
	if (msdc_txfifocnt() || msdc_rxfifocnt()) {
		pr_debug("[SD%d] register abnormal,please check!\n", host->id);
		msdc_reset_hw(host->id);
	}
#endif
	...
	data->error = 0;
	msdc_latest_op[host->id] =
	    (data->flags & MMC_DATA_READ) ? OPER_TYPE_READ : OPER_TYPE_WRITE;
	...
	if (mrq->sbc) {
		...
		if (0 == (host->autocmd & MSDC_AUTOCMD23)) {
			if (msdc_command_start(host, mrq->sbc, CMD_TIMEOUT) !=
			    0)
				return -1;
	...
	if (mrq->cmd->error == (unsigned int)-ETIMEDOUT) {
		if (mrq->cmd->opcode == MMC_SLEEP_AWAKE) {
			if (mrq->cmd->arg & 0x8000) {
				pr_debug(
				    "Sleep_Awake CMD timeout, MSDC_PS %0x\n",
				    MSDC_READ32(MSDC_PS));
				emmc_sleep_failed = 1;
				mrq->cmd->error = 0x0;
				pr_debug("eMMC sleep CMD5 TMO will reinit\n");
			} else {
				host->error |= REQ_CMD_TMO;
			}
		} else {
...
#if (MSDC_DATA1_INT == 1)
	if (host->hw->flags & MSDC_SDIO_IRQ) {
		if ((u_sdio_irq_counter > 0) &&
		    ((u_sdio_irq_counter % 800) == 0))
				   ...
				   struct mmc_request *mrq)
{
	...
	pr_debug(
	    "[%s]: msdc%d, discard task id %d, CMD<%d> arg<0x%08x> rsp<0x%08x>",
	    __func__, host->id, task_id, mmc->deq_cmd.opcode, mmc->deq_cmd.arg,
	    mmc->deq_cmd.resp[0]);

	return mmc->deq_cmd.error;
}

static int msdc_do_request_cq(struct mmc_host *mmc, struct mmc_request *mrq)
{
	struct msdc_host *host = mmc_priv(mmc);
	struct mmc_command *cmd;
#ifdef MTK_MSDC_USE_CACHE
	...
	if (check_mmc_cache_ctrl(host->mmc->card) &&
	    !(cmd->arg & (0x1 << 30))) {
		l_force_prg =
		    !msdc_can_apply_cache(mrq->cmd->arg, cmd->arg & 0xffff);
...
static int tune_cmdq_cmdrsp(struct mmc_host *mmc, struct mmc_request *mrq,
			    int *retry)
{
	struct msdc_host *host = mmc_priv(mmc);
	void __iomem *base = host->base;
	...
	do {
		...
		if (err) {
			...
			pr_debug("msdc%d waiting data transfer done\n",
				 host->id);
		...
		if (status & (1 << 22)) {
			...
			if (state != R1_STATE_TRAN) {
				/* data dma is stop, we need bring card to tran
				 * state */
				if (mmc->is_data_dma == 0) {
					if (state == R1_STATE_DATA ||
					    state == R1_STATE_RCV) {
						ERR_MSG("state<%d> need cmd12 "
							"to stop",
							state);
						msdc_send_stop(host);
					} else if (state == R1_STATE_PRG) {
						ERR_MSG(
						    "state<%d> card is busy",
						    state);
						msleep(100);
					}
				}
				  ...
				  struct mmc_command *sbc)
{
	...
	if (stop) {
		ERR_MSG("XXX STOP<%d><0x%x> Error<%d> Resp<0x%x>", stop->opcode,
			stop->arg, stop->error, stop->resp[0]);
	}

	if (sbc) {
		ERR_MSG("XXX SBC<%d><0x%x> Error<%d> Resp<0x%x>", sbc->opcode,
			sbc->arg, sbc->error, sbc->resp[0]);
	}

#ifdef SDIO_ERROR_BYPASS
	if (is_card_sdio(host) && (host->sdio_error != -EILSEQ) &&
	    (cmd->opcode == 53) &&
...
static int msdc_do_request_async(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (!is_card_present(host) || host->power_mode == MMC_POWER_OFF) {
		ERR_MSG("cmd<%d> arg<0x%x> card<%d> power<%d>",
			mrq->cmd->opcode, mrq->cmd->arg, is_card_present(host),
			host->power_mode);
	...
	if (check_mmc_cmd4647(cmd->opcode))
		arg =
		    mmc->areq_que[((cmd->arg >> 16) & 0x1f)]->mrq_que->cmd->arg;
	else
#endif
		arg = cmd->arg;
				    ...
				    struct mmc_request *mrq)
{
	...
	if (is_card_present(host) && host->power_mode == MMC_POWER_OFF &&
	    ...
	    host->hw->host_function == MSDC_SD) {
		ERR_MSG("cmd<7> arg<0x0> card<1> power<0>, bypass return "
			"-ENOMEDIUM");
	...
	if (mmc->card && mmc_card_cmdq(mmc->card) &&
	    atomic_read(&mmc->areq_cnt) && !check_mmc_cmd001213(cmd->opcode) &&
...
int msdc_error_tuning(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	pr_debug("msdc%d saved device status: %x", host->id,
		 host->device_status);
	/* clear device status */
	...
	if (host->hw->host_function == MSDC_EMMC ||
		...
		case MMC_TIMING_UHS_SDR50:
			pr_debug("msdc%d: SD UHS_SDR104/UHS_SDR50 re-autok %d "
				 "times\n",
		...
		case MMC_TIMING_MMC_HS200:
			if (mmc->ios.clock > MSDC_52M_CLK) {
				pr_debug(
				    "msdc%d: eMMC HS200 re-autok %d times\n",
				    host->id, ++host->reautok_times);
#ifdef CONFIG_MTK_EMMC_CQ_SUPPORT
				...
				if (host->mmc->card &&
					...
					ret = hs200_execute_tuning_cmd(host,
								       NULL);
				else
#endif
		...
		case MMC_TIMING_MMC_HS400:
			if (mmc->ios.clock > MSDC_52M_CLK) {
				pr_debug(
				    "msdc%d: eMMC HS400 re-autok %d times\n",
				    host->id, ++host->reautok_times);
#ifdef CONFIG_MTK_EMMC_CQ_SUPPORT
				/* CQ DAT tune in MMC layer, here tune CMD13 CRC
				 */
				if (host->mmc->card &&
				    mmc_card_cmdq(host->mmc->card))
					ret = hs400_execute_tuning_cmd(host,
								       NULL);
				else
#endif
		...
		default:
			tune_smpl = 1;
			pr_debug("msdc%d: tune smpl %d times, timing:%d, clock "
				 "%dKhz, err: %d\n",
				 host->id, ++host->tune_smpl_times,
				 mmc->ios.timing, mmc->ios.clock / 1000,
				 autok_err_type);
...
void msdc_ops_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (host->mclk != ios->clock) {
		...
		if (ios->timing == MMC_TIMING_MMC_HS400) {
			...
			pr_debug("msdc%d:disable mmc retune\n", host->id);
...
static int msdc_ops_get_cd(struct mmc_host *mmc)
{
	...
	INIT_MSG("Card insert<%d> Block bad card<%d>", host->card_inserted,
		 host->block_bad_card);
	/* spin_unlock_irqrestore(&host->lock, flags); */
	...
	if (hw->flags & MSDC_EXT_SDIO_IRQ) { /* yes for sdio */
		...
		if (enable) {
			while (1) {
				...
				if ((MSDC_READ32(MSDC_INTEN) &
				     MSDC_INT_SDIOIRQ) == 0) {
					pr_debug("Should never ever get into "
						 "this >%d<\n",
						 host->mmc->sdio_irq_pending);
				} else {
					break;
...
static int msdc_ops_switch_volt(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_SIGNAL_VOLTAGE_330:
		pr_debug("%s msdc%d set voltage to 3.3V.\n", __func__,
			 host->id);
		return 0;
	case MMC_SIGNAL_VOLTAGE_180:
		msdc_ungate_clock(host);
		pr_debug("%s msdc%d set voltage to 1.8V.\n", __func__,
			 host->id);
		/* switch voltage */
...
static int msdc_ops_switch_volt_sdio(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->signal_voltage != MMC_SIGNAL_VOLTAGE_330) {
		...
		if ((MSDC_READ32(MSDC_PS) & ((1 << 24) | (0xF << 16))) == 0) {
			...
			while ((status = MSDC_READ32(MSDC_CFG)) &
			       MSDC_CFG_BV18SDT)
				;
...
static void msdc_check_data_timeout(struct work_struct *work)
{
	...
	u32 wints =
	    MSDC_INTEN_XFER_COMPL | MSDC_INTEN_DATTMO | MSDC_INTEN_DATCRCERR;

	if (!data || !mrq || !mmc)
		return;
	pr_notice("[%s]: XXX DMA Data Busy Timeout: %u ms, CMD<%d>", __func__,
		  host->write_timeout_ms, mrq->cmd->opcode);
	...
	if (intsts & wints) {
		pr_notice("[%s]: Warning msdc%d ints are delayed by system, "
			  "ints: %x\n",
	...
	if (msdc_use_async_dma(data->host_cookie) &&
		...
		do {
			...
			if (time_after(jiffies, tmo)) {
				ERR_MSG("card stuck in %d state, remove such "
					"bad card!",
					state);
				spin_unlock(&host->lock);
				if (host->hw->host_function == MSDC_SD)
				   ...
				   struct mmc_data *data, int error)
{
	...
	if ((msdc_use_async_dma(data->host_cookie)) &&
		...
		if (error) {
#ifdef CONFIG_MTK_EMMC_CQ_SUPPORT
			/* CQ mode just set data->error in msdc_irq and return
			 * to mmc tune */
			if (mmc_card_cmdq(host->mmc->card))
				goto skip;
#endif
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	/* E1 32 bit */
	if (mt_get_chip_hw_ver() == 0xca00) {
		host->pio_kaddr = kmalloc(
		    DIV_ROUND_UP((64 * 1024 - 512), PAGE_SIZE) * sizeof(ulong),
		    GFP_KERNEL);
	} else
#endif
	...
	if (pdev->id == 1)
		register_swsusp_restore_noirq_func(
		    ID_M_MSDC, msdc_drv_pm_restore_noirq, &(pdev->dev));
#endif
...
static int msdc_drv_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (is_card_sdio(host) || (host->hw->flags & MSDC_SDIO_IRQ)) {
		...
		if (host->saved_para.suspend_flag == 0) {
			...
			ERR_MSG(
			    "msdc suspend cur_cfg=%x, save_cfg=%x, cur_hz=%d",
...
static int __init mt_msdc_init(void)
{
	...
	if (!wq_init) {
		pr_debug("msdc create work_queue failed.[%s]:%d", __func__,
			 __LINE__);
		WARN_ON(1);
	}
