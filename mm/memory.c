...
static bool pte_map_lock(struct fault_env *fe)
{
	 ...
	 */
	ptl = pte_lockptr(fe->vma->vm_mm, fe->pmd);
...
#else
static inline bool pte_spinlock(struct fault_env *fe)
{
	fe->ptl = pte_lockptr(fe->vma->vm_mm, fe->pmd);
	spin_lock(fe->ptl);
	return true;
...
static inline bool pte_map_lock(struct fault_env *fe)
{
	fe->pte = pte_offset_map_lock(fe->vma->vm_mm, fe->pmd,
				       fe->address, &fe->ptl);
	return true;
 ...
 */
static inline int pte_unmap_same(struct fault_env *fe)
{
	int ret = 0;
	...
	if (sizeof(pte_t) > sizeof(unsigned long)) {
		if (pte_spinlock(fe)) {
			if (!pte_same(*fe->pte, fe->orig_pte))
	...
	__releases(fe->ptl)
{
	...
	old_page = __vm_normal_page(vma, fe->address, orig_pte, fe->vma_flags);
	if (!old_page) {
		/*
...
/*
	...
	if (p4d_none(p4dval) || unlikely(p4d_bad(p4dval)))
		goto out_walk;
*/
	fe.pud = pud_offset(pgd, address);
	pudval = READ_ONCE(*fe.pud);
...
/*
	if (unlikely(pud_trans_huge(pudval)))
		goto out_walk;
*/
	fe.pmd = pmd_offset(fe.pud, address);
	...
	fe.vma = vma;
/*
	fe.pgoff = linear_page_index(vma, address);
	fe.gfp_mask = __get_fault_gfp_mask(vma);
*/
	fe.sequence = seq;
	fe.flags = flags;
